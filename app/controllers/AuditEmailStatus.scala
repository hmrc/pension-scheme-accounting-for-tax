...
trait AuditEmailStatus {
  ...
  private def decryptPsaOrPspIdAndEmail(encryptedPsaOrPspId: String, email: String): Either[String, EmailIdentifiers] = {
    val psaOrPspId = crypto.decrypt(Crypted(encryptedPsaOrPspId)).value
    val emailAddress = crypto.decrypt(Crypted(email)).value

    if (emailAddress.matches(emailRegex)) {
      Right(EmailIdentifiers(psaOrPspId, emailAddress))
    } else {
      Left(emailAddress)
    }
  }
  ...
  protected def auditEmailStatus(requestId: String,
    ...
    decryptPsaOrPspIdAndEmail(encryptedPsaOrPspId, email)
      .fold(invalidEmailAddress => Forbidden(s"Malformed email: $invalidEmailAddress"),
        emailIdentifiers => request.body.validate[EmailEvents] match {
          case JsSuccess(valid, _) =>
            valid.events.filterNot(
              ...
              auditService.sendEvent(
                EmailAuditEvent(
                  emailIdentifiers.psaOrPspId,
                  submittedBy,
                  emailIdentifiers.emailAddress,
                  event.event,
                  journeyType,
                  requestId
                ))(request, implicitly)
            ...
            Ok
          case JsError(errors) => BadRequest("Bad request received for email call back event")
        }
      )
  }
}
