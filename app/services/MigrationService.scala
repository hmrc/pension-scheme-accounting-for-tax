...
class MigrationService @Inject()(mongoLockRepository: MongoLockRepository,
  ...
  private def fixExpireAt(collectionName: String) = {
    ...
    logger.warn("[PODS-9319] Started file upload reference data migration for " + collectionName + " collection")
    collection.find(BsonDocument("expireAt" -> BsonDocument("$type" -> BsonString("string")))).toFuture().flatMap { seq =>
      val ftr = Future.sequence(seq.map(item => {
        ...
        val modifier = Updates.set("expireAt", BsonDateTime(java.util.Date.from(Instant.now().plusMillis(1000 * 60 * 5))))
  ...
  lock withLock {
    for {
      res <- fixExpireAt("file-upload-response")
    } yield res
  } map {
    case Some(result) =>
