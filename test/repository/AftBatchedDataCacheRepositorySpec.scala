...
class AftBatchedDataCacheRepositorySpec extends AnyWordSpec with MockitoSugar
  with Matchers with MongoEmbedDatabase with BeforeAndAfter with BeforeAndAfterEach {
  ...
  "save" must {
    "save de-reg charge batch correctly in Mongo collection where there is some session data" in {
      ...
      when(batchService.batchIdentifierForChargeAndMember(any(), any())).thenReturn(Some(BatchIdentifier(BatchType.Other, 1)))
      when(batchService.getOtherJsObject(ArgumentMatchers.eq(fullPayload))).thenReturn(dummyJson)

      val result = Await.result(
        aftBatchedDataCacheRepository.save(id, sessionId,
          Option(ChargeAndMember(ChargeType.ChargeTypeDeRegistration, None)), fullPayload),
        Duration.Inf)

      result mustBe true
      whenReady(aftBatchedDataCacheRepository
        .find("uniqueAftId" -> uniqueAftId)) { documentsInDB =>
        documentsInDB.size mustBe 12
        val otherBatch = documentsInDB.find( jsValue => (jsValue \ "batchType").asOpt[String].contains(BatchType.Other.toString))
    ...
    "save member-based charge batch for a member correctly in Mongo collection where there is some session data" in {
      ...
      when(batchService.batchIdentifierForChargeAndMember(any(), any())).thenReturn(Some(BatchIdentifier(BatchType.ChargeC, 4)))
      when(batchService.getChargeTypeJsObjectForBatch(any(), any(), any(), any())).thenReturn(amendedBatchInfo)

      val result = Await.result(
        aftBatchedDataCacheRepository.save(id, sessionId,
          Option(ChargeAndMember(ChargeType.ChargeTypeAuthSurplus, Some(4))), fullPayload),
        Duration.Inf)
      ...
      whenReady(aftBatchedDataCacheRepository
        .find("uniqueAftId" -> uniqueAftId)) { documentsInDB =>
        documentsInDB.size mustBe 12
        val chargeCBatch = documentsInDB.find( jsValue =>
          (jsValue \ "batchType").asOpt[String].contains(BatchType.ChargeC.toString) &&
          (jsValue \ "batchNo").asOpt[Int].contains(2)
        )
    ...
    "remove all documents if there is no session data batch (i.e. it has expired)" in {
      ...
      val result = Await.result(
        aftBatchedDataCacheRepository.save(id, sessionId,
          Option(ChargeAndMember(ChargeType.ChargeTypeDeRegistration, None)), payloadOther),
        Duration.Inf)
      result mustBe false
      whenReady(aftBatchedDataCacheRepository
        .find("uniqueAftId" -> uniqueAftId)) { documentsInDB =>
  ...
  "setSessionData" must {
    "save all batches correctly in Mongo collection, removing batches beyond charge G batch 3" in {
      ...
      val result = Await.result(
        aftBatchedDataCacheRepository.setSessionData(
          id, lockDetail, dummyJson, sessionId,
          version = version, accessMode = accessMode, areSubmittedVersionsAvailable = areSubmittedVersionsAvailable),
        Duration.Inf)
      ...
      whenReady(aftBatchedDataCacheRepository.find("uniqueAftId" -> uniqueAftId)) { result =>
        ...
        result.foreach{ b =>
          BatchType.getBatchType((b \ "batchType").as[String]).map { batchType =>
            checkResult(batchType, (b \ "batchNo").asOpt[Int].getOrElse(0), (b \ "data").get, fullSetOfBatchesToSaveToMongo)
          }
        }
      }
    }
  ...
  "get" must {
    ...
    "remove all documents and return None if there is no session data batch (i.e. it has expired)" in {
      ...
      result mustBe None
      whenReady(aftBatchedDataCacheRepository.find("uniqueAftId" -> uniqueAftId)){_.size mustBe 0}
    }
  ...
  "getSessionData" must {
    ...
    "remove all documents and return None if there is no session data batch (i.e. it has expired)" in {
      ...
      result mustBe None
      whenReady(aftBatchedDataCacheRepository
        .find("uniqueAftId" -> uniqueAftId)){_.size mustBe 0}
    }
  }
...
object AftBatchedDataCacheRepositorySpec extends AnyWordSpec with MockitoSugar {
  ...
  private def mongoCollectionDrop():Boolean =
    Await.result(aftBatchedDataCacheRepository.collection.drop(false), Duration.Inf)

  private def mongoCollectionInsertBatches(id:String, sessionId: String, s:Seq[BatchInfo]):Boolean = {
    ...
    val ff = s.map { bi =>
      val modifier = BSONDocument.apply("$set" ->
        Json.obj(
          "id" -> id,
          "id" -> sessionId,
          "data" -> bi.jsValue
  ...
  def checkResult(batchType:BatchType, batchNo:Int, jsValue:JsValue, expectedBatches:Set[BatchInfo]):scalatest.Assertion = {
    (batchType, batchNo) match {
      case (BatchType.Other, 1) =>
        val expectedBatch = expectedBatches.find(b => b.batchType == batchType && b.batchNo == batchNo)
          .map( _.jsValue.as[JsObject] ).getOrElse(Json.obj())
        jsValue mustBe expectedBatch
      case (BatchType.SessionData, 1) =>
        jsValue.as[JsObject] mustBe Json.obj(
          "sessionId" -> sessionId,
          "lockDetail" -> Json.toJson(lockDetail),
          "version" -> version,
          "accessMode" ->  accessMode,
