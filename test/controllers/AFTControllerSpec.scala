...
import utils.{InvalidPayloadHandlerImpl, JsonFileReader, ValidationFailure}
...
class AFTControllerSpec extends AsyncWordSpec with Matchers with MockitoSugar with BeforeAndAfter with JsonFileReader {
  ...
  private val mockInvalidPayloadHandlerImpl = mock[InvalidPayloadHandlerImpl]
  ...
  val modules: Seq[GuiceableModule] =
    Seq(
      ...
      bind[InvalidPayloadHandlerImpl].toInstance(mockInvalidPayloadHandlerImpl)
    )
  ...
  before {
    ...
    when(mockInvalidPayloadHandlerImpl.getFailures(any())(any())).thenReturn(Set.empty)
  }
  ...
  "fileReturn" must {
    ...
    "return invalid payload errors when invalid payload generated by transformation" in {
      ...
      val failures = Set(
        ValidationFailure("test1", "test", None),
        ValidationFailure("test2", "test", None)
      )
      when(mockInvalidPayloadHandlerImpl.getFailures(any())(any())).thenReturn(failures)
      ...
      recoverToExceptionIf[AFTValidationFailureException] {
        ...
        val expectedFailures = {
          val mapped = failures.map { e =>
            s"Message: ${e.message}, FailureType:${e.failureType}, Value:${e.value.getOrElse("")}"
          }
          s"Invalid AFT file AFT return:-\n${mapped.mkString}"
        }

        ex.exMessage mustBe expectedFailures
      }
    }
