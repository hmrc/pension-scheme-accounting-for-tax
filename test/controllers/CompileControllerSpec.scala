...
class CompileControllerSpec extends AsyncWordSpec with MustMatchers with MockitoSugar with BeforeAndAfter {
  ...
  private val controller = new CompileController(appConfig, stubControllerComponents(), mockDesConnector)
  private val json = Json.obj(fields = "testId" -> "data")
  ...
  "Compile" should {
    "return OK when valid response from DES" in {
      running(app) {
        ...
        val result = controller.fileReturn()(fakeRequest.withJsonBody(json).withHeaders(newHeaders = "pstr" -> pstr))
    ...
    "throw Upstream5XXResponse on Internal Server Error from DES" in {
      running(app) {
        ...
        recoverToExceptionIf[Upstream5xxResponse] {
          controller.fileReturn()(fakeRequest.withJsonBody(json).withHeaders(newHeaders = "pstr" -> pstr))
        } map {
