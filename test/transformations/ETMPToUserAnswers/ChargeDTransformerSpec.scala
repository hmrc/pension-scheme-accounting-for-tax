    ...
    "must transform ChargeDDetails from ETMP format to UserAnswers format" in {
      forAll(chargeDETMPGenerator) {
        etmpResponseJson =>
          ...
          (membersUAPath(0) \ "chargeDetails" \ "taxAt25Percent").as[BigDecimal].mustBe((membersETMPPath(0) \ "totalAmountDueAtLowerRate").as[BigDecimal])
          (membersUAPath(0) \ "chargeDetails" \ "taxAt55Percent").as[BigDecimal].mustBe((membersETMPPath(0) \ "totalAmountDueAtHigherRate").as[BigDecimal])
          ...
          isMcCloudRem match {
            case Some(true) =>
              ...
              areMorePensions match {
                case Some(true) =>
                  (membersUAPath(0) \ "mccloudRemedy" \ "schemes" \ 0 \ "pstr").as[String]
                    .mustBe((membersETMPPath(0) \ "pensionSchemeDetails" \ 0 \ "pstr").as[String])
                  (membersUAPath(0) \ "mccloudRemedy" \ "schemes" \ 0 \ "chargeAmountReported").as[BigDecimal]
                    .mustBe((membersETMPPath(0) \ "pensionSchemeDetails" \ 0 \ "amtOrRepLtaChg").as[BigDecimal])
                  val date = (membersETMPPath(0) \ "pensionSchemeDetails" \ 0 \ "repPeriodForLtac").as[String]
                  (membersUAPath(0) \ "mccloudRemedy" \ "schemes" \ 0 \"taxYearReportedAndPaidPage").as[String]
                    .mustBe(formatDateDMYString(date).getYear.toString)
                  (membersUAPath(0) \ "mccloudRemedy" \ "schemes" \ 0 \"taxQuarterReportedAndPaid" \ "startDate").as[String]
                  .mustBe(getQuarterStartDate(formatDateDMYString(date).toString))
                  (membersUAPath(0) \ "mccloudRemedy" \ "schemes" \ 0 \"taxQuarterReportedAndPaid" \ "endDate").as[String]
                    .mustBe(formatDateDMYString(date).toString)
                  (membersUAPath(0) \ "mccloudRemedy" \ "isChargeInAdditionReported").as[Boolean].mustBe(true)

                case Some(false) =>
                  (membersUAPath(0) \ "mccloudRemedy" \ "chargeAmountReported").as[BigDecimal]
                    .mustBe((membersETMPPath(0) \ "pensionSchemeDetails" \ 0 \ "amtOrRepLtaChg").as[BigDecimal])
                  val date = (membersETMPPath(0) \ "pensionSchemeDetails" \ 0 \ "repPeriodForLtac").as[String]
                  (membersUAPath(0) \ "mccloudRemedy" \"taxYearReportedAndPaidPage").as[String]
                    .mustBe(formatDateDMYString(date).getYear.toString)
                  (membersUAPath(0) \ "mccloudRemedy" \"taxQuarterReportedAndPaid" \ "startDate").as[String]
                    .mustBe(getQuarterStartDate(formatDateDMYString(date).toString))
                  (membersUAPath(0) \ "mccloudRemedy" \"taxQuarterReportedAndPaid" \ "endDate").as[String]
                    .mustBe(formatDateDMYString(date).toString)
                  (membersUAPath(0) \ "mccloudRemedy" \ "isChargeInAdditionReported").as[Boolean].mustBe(true)
