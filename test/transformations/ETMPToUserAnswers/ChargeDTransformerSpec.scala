    ...
    "must transform ChargeDDetails from ETMP format to UserAnswers format" in {
      forAll(chargeDETMPGenerator) {
        etmpResponseJson =>
          ...
          (membersUAPath(0) \ "chargeDetails" \ "taxAt25Percent").as[BigDecimal] mustBe (membersETMPPath(0) \ "totalAmountOfTaxDueAtLowerRate").as[BigDecimal]
          (membersUAPath(0) \ "chargeDetails" \ "taxAt55Percent").as[BigDecimal] mustBe (membersETMPPath(0) \ "totalAmountOfTaxDueAtHigherRate").as[BigDecimal]
